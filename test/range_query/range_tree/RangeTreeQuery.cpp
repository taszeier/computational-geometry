...
#include "range_query/range_tree/RangeTreeQuery.hpp"

using namespace compg;

TEST_CASE(
    "RangeTreeQuery - vertices collinear to orthogonal range query lines are "
    "excluded",
    "[RangeTreeQuery]"
) {
    ...
    RangeTree<2> tree{vertices};
    RangeTreeQuery<2> query;

    SECTION("Query region covers bottom left corner") {
        auto queryResult = query.Query(tree, RangeTreeQueryRegion<2>{{{0, 0}, {1, 1}}});

        std::ranges::sort(queryResult, LexicographicalLess<Vertex<2>>{}, ValueProjector{});
        REQUIRE(queryResult.size() == 4);
        REQUIRE(AreEqual(queryResult[0].Value, {0, 0}));
        REQUIRE(queryResult.at(0).Index == 0);
        REQUIRE(AreEqual(queryResult[1].Value, {0, 1}));
        REQUIRE(queryResult.at(1).Index == 1);
        REQUIRE(AreEqual(queryResult[2].Value, {1, 0}));
        REQUIRE(queryResult.at(2).Index == 3);
        REQUIRE(AreEqual(queryResult[3].Value, {1, 1}));
        REQUIRE(queryResult.at(3).Index == 4);
    }

    SECTION("Query region covers top left corner") {
        auto queryResult = query.Query(tree, RangeTreeQueryRegion<2>{{{0, 1}, {1, 2}}});
    ...
    SECTION("Query region covers top right corner") {
        auto queryResult = query.Query(tree, RangeTreeQueryRegion<2>{{{1, 1}, {2, 2}}});
    ...
    SECTION("Query region covers bottom right corner") {
        auto queryResult = query.Query(tree, RangeTreeQueryRegion<2>{{{1, 0}, {2, 1}}});
...
TEST_CASE("RangeTreeQuery - query region covers all vertices", "[RangeTreeQuery]") {
    const std::vector<Vertex2D> vertices{{1, 1}, {1, 2}, {1.5, 1.5}};
    RangeTree<2> tree{vertices};
    RangeTreeQuery<2> query;

    SECTION("Query region covers all vertices") {
        auto queryResult = query.Query(tree, RangeTreeQueryRegion<2>{{{0, 0}, {2, 2}}});
...
TEST_CASE("RangeTreeQuery - query region does not cover any vertices", "[RangeTreeQuery]") {
    const std::vector<Vertex2D> vertices{{1, 1}, {3, 3}};
    RangeTree<2> tree{vertices};
    RangeTreeQuery<2> query;

    SECTION("Orthogonal query region covers zero vertices") {
        auto queryResult = query.Query(tree, RangeTreeQueryRegion<2>{{{1.5, 1.5}, {2.5, 2.5}}});
