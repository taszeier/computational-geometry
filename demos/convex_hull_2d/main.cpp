...
int main(int argc, char* argv[]) {
    ...
    QMainWindow window;
    window.setWindowTitle("Computational Geometry - 2D Convex Hull");
    ...
    auto* shortcuts = new QLabel{
        "Left click: add point    Wheel: zoom    Right drag: pan    Ctrl+C: clear    Ctrl+Q: quit", content
    };
    shortcuts->setStyleSheet("color: #475569;");

    auto* canvas = new ConvexHullWidget{content};
    layout->addWidget(description);
    layout->addWidget(shortcuts);
    layout->addWidget(canvas, 1);
    window.setCentralWidget(content);
    window.statusBar()->showMessage("Click the drawing to add a point");
    ...
    QObject::connect(&clearShortcut, &QShortcut::activated, canvas, &ConvexHullWidget::clear);

    QObject::connect(canvas, &ConvexHullWidget::clicked, &window, [&window](const QPointF& position) {
        window.statusBar()->showMessage(
            QString{"Point at (%1, %2)"}.arg(position.x(), 0, 'f', 2).arg(position.y(), 0, 'f', 2)
        );
    });
